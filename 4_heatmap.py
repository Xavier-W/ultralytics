...
class yolov8_heatmap:
    ...
    def draw_detections(self, box, color, name, img):
        ...
        cv2.putText(img, str(name), (xmin, ymin - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,0,255), 2, lineType=cv2.LINE_AA)
        return img
    ...
    def __call__(self, img_path, save_path):
        ...
        if os.path.isdir(img_path):
            ...
            self.process(img_path, save_path+os.path.basename(img_path))
        
def get_params():
    params = {
        'weight': './ultralytics/landslide_exp3/weights/best.pt', # 现在只需要指定权重即可,不需要指定cfg
        'device': 'cuda:0',
...
if __name__ == '__main__':
    ...
    model(r'./7.png', "./0_outputs/heatmap_")